...
class IERecruitertypeController extends BackendController
{
    public function getImport() {
    	return view('backend.import-export.recruitertype.import');
    }
    ...
    public function postImport(Request $request) {
    	...
    	foreach($rows as $row) {
    		if(isset($row[0]) && !empty($row[0])) {
                ...
                if($name){
        			if(ImportRepo::addRecruiterType($row[0])) {
        				$successCount++;
        			} else {
        				$skipedRows[] = $row;
        			}
    ...
    public function getExport() {
    	return view('backend.import-export.recruitertype.export');
    }
    ...
    public function postExport(Request $request) {
    	...
    	$data=ImportRepo::getAllRecruiterType($isDate,$startDate,$endDate);
        if($data){
        	$filename = $this->writeCsv($data);

        	$file = storage_path().'/app/'.$filename;
        	$current_date = \Carbon\Carbon::now()->format('d_m_Y');
            return Response::download($file, "recruitertype_".$current_date.".csv", ['content-type' => 'text/csv']); 
        }else{
            return redirect()->back()->with([
    ...
    public function getSample(Request $request){
        ...
        $file = storage_path().'/app/import-sample/recruitertype_sample.csv';
        return Response::download($file, 'recruitertype_sample.csv', ['content-type' => 'text/csv']);        
    }
}
