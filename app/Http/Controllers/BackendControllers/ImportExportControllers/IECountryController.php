...
class IECountryController extends BackendController
{
    
    public function getImport() {
        return view('backend.import-export.country.import');
    }
    ...
    public function postImport(Request $request) {
    	...
    	foreach($rows as $row) {
    		if(isset($row[0]) && !empty($row[0])) {
    			...
    			if($name){
                    if(ImportRepo::addCountry($row[0])) {
        				$successCount++;
        			} else {
        				$skipedRows[] = $row;
        			}
    ...
    public function getExport() {
    	return view('backend.import-export.country.export');
    }
    ...
    public function postExport(Request $request) {
    	...
    	$data=ImportRepo::getAllCountry($isDate,$startDate,$endDate);
        ...
        if($data){
            ...
            return Response::download($file, "countries_".$current_date.".csv", ['content-type' => 'text/csv']); 
        }else{
            return redirect()->back()->with([
    ...
    public function getSample(Request $request){
        ...
        $file = storage_path().'/app/import-sample/country_sample.csv';
        return Response::download($file, 'country_sample.csv', ['content-type' => 'text/csv']);        
    }
