...
class IEExperienceLevelController extends BackendController
{
    public function getImport() {
    	return view('backend.import-export.experience-level.import');
    }
    ...
    public function postImport(Request $request) {
    	...
    	foreach($rows as $row) {
    		if(isset($row[0]) && !empty($row[0])) {
                ...
                if($name){
        			if(ImportRepo::addExperienceLevel($row[0])) {
        				$successCount++;
        			} else {
        				$skipedRows[] = $row;
        			}
    ...
    public function getExport() {
    	return view('backend.import-export.experience-level.export');
    }
    ...
    public function postExport(Request $request) {
    	...
    	$data=ImportRepo::getAllExperienceLevel($isDate,$startDate,$endDate);
        if($data){
        	$filename = $this->writeCsv($data);

        	$file = storage_path().'/app/'.$filename;
        	$current_date = \Carbon\Carbon::now()->format('d_m_Y');
            return Response::download($file, "experiencelevel_".$current_date.".csv", ['content-type' => 'text/csv']); 
        }else{
            return redirect()->back()->with([
    ...
    public function getSample(Request $request){
        ...
        $file = storage_path().'/app/import-sample/experiencelevel_sample.csv';
        return Response::download($file, 'experiencelevel_sample.csv', ['content-type' => 'text/csv']);        
    }
}
