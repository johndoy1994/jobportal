...
class IEEducationController extends BackendController
{
    public function getImport() {
        return view('backend.import-export.education.import');
    }
    ...
    public function postImport(Request $request) {
    	...
    	foreach($rows as $row) {
    		if(isset($row[0]) && !empty($row[0])) {
                $name=Utility::education_validation($row[0]);
                if($name){
        			if(ImportRepo::addEducation($row[0])) {
        				$successCount++;
        			} else {
        				$skipedRows[] = $row;
        			}
    ...
    public function getExport() {
    	return view('backend.import-export.education.export');
    }
    ...
    public function postExport(Request $request) {
    	...
    	$data=ImportRepo::getAllEducation($isDate,$startDate,$endDate);
        if($data){
        	$filename = $this->writeCsv($data);

        	$file = storage_path().'/app/'.$filename;
        	$current_date = \Carbon\Carbon::now()->format('d_m_Y');
            return Response::download($file, "education_".$current_date.".csv", ['content-type' => 'text/csv']); 
        }else{
            return redirect()->back()->with([
    ...
    public function getSample(Request $request){
        ...
        $file = storage_path().'/app/import-sample/education_sample.csv';
        return Response::download($file, 'education_sample.csv', ['content-type' => 'text/csv']);        
    }
}
