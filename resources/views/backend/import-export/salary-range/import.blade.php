...
@section('title', 'Import Salary Range')

@section('content')
	<div id="page-wrapper">
		<div class="row padding-top-10">
			<div class="col-md-9">
				<h3>Import Salary Range</h3>
			</div>
			<div class="col-md-3 text-right padding-top-10">
				...
				<a href="{{route('import-salary-range-sample')}}" class="btn btn-primary pull-right">Sample CSV</a>
			</div>
		</div>
		<hr/>
		<div class="row">
			<div class="col-md-12">
				<form class="well form-horizontal" method="post" action="{{route('import-salary-range-post')}}" enctype="multipart/form-data">
					<legend>Import Salary Range</legend>
					...
					<fieldset>
						<div class="form-group">
							<label class="control-label col-lg-3">Salary Type Name :</label>
							<div class="col-lg-5">
								<select class="form-control" id="salary_type_id" name="salary_type_id" required="">
									<option value="">Select job category</option>
									@foreach($salarytype as $salarytype)
										<option value="{{$salarytype->id}}">{{$salarytype->salary_type_name}}</option>
									@endforeach
								</select>
							</div>
						</div>
